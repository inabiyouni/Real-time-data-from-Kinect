...
LiveScanClient::LiveScanClient() :
	...
	m_pMatlabSocket(NULL), // just testing
			...
			if (IDC_BUTTON_CONNECT == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
			{
				...
				else
				{
					...
					try
					{
						...
						m_pMatlabSocket = new SocketClient(address, 48002); // just testing
						m_bConnected = true;
...
void LiveScanClient::StoreFrame(Point3f *vertices, Point2f *mapping, RGB *color, vector<Body> &bodies, BYTE* bodyIndex)
{
	...
	// my added code to send the data to an extra TCP/IP reader
	for (int i = 0; i < 6; i++)
	{
		if (m_vLastFrameBody[i].bTracked)
		{

			int nBodiesT = m_vLastFrameBody.size();
			int sizeMat = 0;
			sizeMat += sizeof(nBodiesT);
			//Send one body to make it faster//for (int i = 0; i < nBodiesT; i++)
			{
				sizeMat += sizeof(m_vLastFrameBody[i].bTracked);
				int nJointsT = m_vLastFrameBody[i].vJoints.size();
				sizeMat += sizeof(nJointsT);
				sizeMat += nJointsT * (3 * sizeof(float)+2 * sizeof(int));
				sizeMat += nJointsT * 2 * sizeof(float);
			//}


			vector<char> bufferMat(sizeMat);
			//char *ptr2 = (char*)vertices.data();
			int posMat = 0;

			memcpy(bufferMat.data() + posMat, &nBodiesT, sizeof(nBodiesT));
			posMat += sizeof(nBodiesT);

			//for (int i = 0; i < nBodiesT; i++)
			//{
				memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].bTracked, sizeof(m_vLastFrameBody[i].bTracked));
				posMat += sizeof(m_vLastFrameBody[i].bTracked);

				//int nJoints = m_vLastFrameBody[i].vJoints.size();
				memcpy(bufferMat.data() + posMat, &nJointsT, sizeof(nJointsT));
				posMat += sizeof(nJointsT);

				for (int j = 0; j < nJointsT; j++)
				{
					//Joint
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJoints[j].JointType, sizeof(JointType));
					posMat += sizeof(JointType);
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJoints[j].TrackingState, sizeof(TrackingState));
					posMat += sizeof(TrackingState);
					//Joint position
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJoints[j].Position.X, sizeof(float));
					posMat += sizeof(float);
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJoints[j].Position.Y, sizeof(float));
					posMat += sizeof(float);
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJoints[j].Position.Z, sizeof(float));
					posMat += sizeof(float);

					//JointInColorSpace
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJointsInColorSpace[j].X, sizeof(float));
					posMat += sizeof(float);
					memcpy(bufferMat.data() + posMat, &m_vLastFrameBody[i].vJointsInColorSpace[j].Y, sizeof(float));
					posMat += sizeof(float);
				}
				if (!m_bConnected)
				{
				...
				//m_pMatlabSocket->SendBytes((char*)&sizeMat, sizeof(int));
				m_pMatlabSocket->SendBytes(bufferMat.data(), sizeMat);
				break;
			}
